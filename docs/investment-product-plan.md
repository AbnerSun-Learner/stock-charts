...
- 行情与基准为全局共享只读数据，不按用户复制。物理表复用已有 `etf_daily`、`etf_pool_snapshots`、`indices`、`index_daily_prices`；汇率使用 `fx_rates`（`scheduled-tasks` 已建 DDL + Frankfurter 同步）。**不新建**同义的 `price_bars` / `benchmarks` / `benchmark_prices`（领域类型仍可用 `PriceBar` 等命名，由应用层映射）。
- **双仓分工（强制）**：建表 / migration、第三方数据源接入、共享表批量落库统一在 **`scheduled-tasks`**；本仓库（`stock-charts`）只做业务实现与经 Supabase 客户端的账本读写 / 共享表只读。账本业务数据由用户在 UI 填写。认证 Phase 1 使用**邮箱 Magic Link**。
- 第一阶段账本以 CSV/手工导入保证可控；日 K / 汇率优先读已有同步数据。不接券商账户，不做自动交易。
...
- 共享日 K / 指数 / 汇率由 `scheduled-tasks` 同步落表；本仓只读。账本第一阶段仍以手动/CSV 保证可控。不新建同义行情表。
