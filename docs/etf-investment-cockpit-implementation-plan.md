...
组合设置的交互约定：组合基准代码必须从数据库中的共享 ETF 池和用户自定义 ETF
代码和对应中文名称下拉选择，保存值为规范化 ETF `symbol`，禁止手工录入任意代码。现金目标权重在
界面以百分比输入（例如 `20%`），写入 `portfolio_settings.cash_target_weight` 时转换为
领域层约定的 `0.2`。
