...
export function DrawdownPanel({ points, loading, error }: { points: IndexMetricPoint[]; loading: boolean; error: string | null }) {
  ...
  return <PanelShell title="极限跌幅" eyebrow="Drawdown stress">
    {loading ? <PanelState message="正在计算回撤…" tone="loading" /> : error ? <PanelState message={error} tone="error" /> : !summary ? <PanelState message="该指数暂无可计算的收盘数据" /> : <div className="grid gap-4 sm:grid-cols-2">
